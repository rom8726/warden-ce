...
func renderMessage(issue *domain.Issue, project *domain.Project, baseURL string, isRegress bool) (string, error) {
	...
	const msgTemplate = `<b>[{{.ProjectName}}] {{.IssueTitle}}</b>
<b>Level:</b> {{.IssueLevel}}
<b>Is Regress:</b> {{if .IsRegress}}Yes{{else}}No{{end}}
<b>First Seen:</b> {{.FirstSeen}}
<b>Last Seen:</b> {{.LastSeen}}
<b>Status:</b> {{.IssueStatus}}
<b>Platform:</b> {{.IssuePlatform}}
<b>Occurrences:</b> {{.IssueOccurrences}}
<b>URL:</b> {{.IssueURL}}`
